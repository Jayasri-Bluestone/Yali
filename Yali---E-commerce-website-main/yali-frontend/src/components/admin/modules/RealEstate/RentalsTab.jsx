...
import { Home, MapPin, Key, DollarSign, Plus, Filter, Edit, Trash2, Search, Download } from 'lucide-react';
...
const rentalsSchema = [
  ...
  { name: 'propertyType', label: 'Property Type (House/Apt/Commercial)', type: 'text', required: true },
...
export function RentalsTab() {
  ...
  const fetchProperties = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/products?category=RealEstate&sub_category=Rentals&all=true`);
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Rentals</h2>
          <p className="text-gray-500 font-medium mt-1">Manage rental properties and leases</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button 
            onClick={() => exportToCSV(filteredProperties, 'rentals')}
          ...
          <button onClick={() => handleOpenModal()} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold rounded-xl shadow-sm hover:shadow-md transition-all">
        ...
        {['All', 'Active', 'Inactive'].map(tab => (
          <button 
            ...
            className={`pb-3 font-bold text-sm transition-all border-b-2 ${
              filter === tab ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-900'
            }`}
          >
            {tab} Listings
          </button>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-200">
                  ...
                  <th className="py-4 px-6 font-bold text-gray-600 text-sm uppercase tracking-wider">Rental Terms</th>
                ...
                {paginatedProperties.map(property => {
                  ...
                  return (
                    <tr key={property.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <div className="font-black text-gray-900">₹{parseFloat(property.price).toLocaleString()} <span className="text-sm font-medium text-gray-500">/mo</span></div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex flex-col gap-1">
                           <div className="flex items-center gap-2 text-xs text-gray-600">
                             <DollarSign className="w-3 h-3 text-orange-500" /> <span className="font-medium">Deposit: ₹{meta.deposit || 0}</span>
                           </div>
                           <div className="flex items-center gap-2 text-xs text-gray-600">
                             <Key className="w-3 h-3 text-blue-500" /> <span className="font-medium">Ready to move</span>
                           </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
