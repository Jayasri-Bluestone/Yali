...
import { Map, MapPin, Maximize, Droplet, Sprout, Plus, Filter, Edit, Trash2, Search, Download } from 'lucide-react';
import { exportToCSV } from '../../../../utils/csvExport';
import { Pagination } from '../../Pagination';
import { CategoryProductModal } from '../Shared/CategoryProductModal';
import { API_URL } from '../../../../config';

const agriculturalSchema = [
  { name: 'location', label: 'Location / Area', type: 'text', required: true },
  { name: 'areaSize', label: 'Total Area (Acres/Hectares)', type: 'text', required: true },
  { name: 'soilType', label: 'Soil Type', type: 'text', required: true },
  { name: 'waterSource', label: 'Water Source Availability', type: 'text', required: true },
  { name: 'agent', label: 'Listing Agent Name', type: 'text', required: true }
];

export function AgriculturalLandTab() {
  ...
  const fetchProperties = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/products?category=RealEstate&sub_category=AgriculturalLand&all=true`);
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Agricultural Land</h2>
          <p className="text-gray-500 font-medium mt-1">Manage farms, ranches, and agricultural empty land</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button 
            onClick={() => exportToCSV(filteredProperties, 'agricultural_land')}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-200">
                  ...
                  <th className="py-4 px-6 font-bold text-gray-600 text-sm uppercase tracking-wider">Land Specifics</th>
                    ...
                    <tr key={property.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        ...
                        <div className="flex items-center gap-2 text-xs font-medium text-gray-500 mt-1">
                          <span className="flex items-center gap-1"><Maximize className="w-3 h-3 text-gray-400" /> {meta.areaSize || '0 Acres'}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex flex-col gap-1">
                           <div className="flex items-center gap-1 text-xs text-gray-600">
                             <Sprout className="w-3 h-3 text-emerald-500" /> <span className="font-medium">{meta.soilType || 'N/A'}</span>
                           </div>
                           <div className="flex items-center gap-1 text-xs text-gray-600">
                             <Droplet className="w-3 h-3 text-blue-500" /> <span className="font-medium">{meta.waterSource || 'N/A'}</span>
                           </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
      ...
      <CategoryProductModal
        ...
        category="RealEstate"
        subCategory="AgriculturalLand"
        schema={agriculturalSchema}
