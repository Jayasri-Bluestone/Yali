...
import { Car, Fuel, Settings2, Calendar, MapPin, MoreVertical, Plus, Filter, Gauge, Edit, Trash2 } from 'lucide-react';
...
const carSchema = [
  ...
  { name: 'fuel', label: 'Fuel Type', type: 'select', options: ['Petrol', 'Diesel', 'Electric', 'Hybrid', 'CNG'], required: true },
  { name: 'transmission', label: 'Transmission', type: 'select', options: ['Automatic', 'Manual'], required: true },
  { name: 'body_type', label: 'Body Type', type: 'select', options: ['SUV', 'Sedan', 'Hatchback', 'MUV', 'Luxury'], required: true },
...
export function CarsTab() {
  ...
  const fetchCars = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/products?category=Automobiles&sub_category=Cars&all=true`);
  ...
  const filteredCars = cars.filter(c => filter === 'All' || c.status.toLowerCase() === filter.toLowerCase());

  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Cars & SUVs</h2>
          <p className="text-gray-500 font-medium mt-1">Manage all four-wheeler listings</p>
        </div>
                ...
                {filteredCars.map(car => {
                  ...
                  return (
                    <tr key={car.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <div className="font-black text-gray-900">₹{parseFloat(car.price).toLocaleString()}</div>
                        <div className="flex items-center gap-2 text-xs font-medium text-gray-500 mt-1">
                          ...
                          <span>•</span>
                          <span className="flex items-center gap-1"><Fuel className="w-3 h-3 text-gray-400" /> {meta.fuel || 'N/A'}</span>
                          <span>•</span>
                          <span className="flex items-center gap-1 text-gray-500">{meta.transmission || 'N/A'}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-4">
                          <div className="text-center">
                            <div className="text-sm font-bold text-gray-900">1.8k</div>
                      ...
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleOpenModal(car)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
