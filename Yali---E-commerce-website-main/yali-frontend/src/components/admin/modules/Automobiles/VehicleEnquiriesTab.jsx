...
export function VehicleEnquiriesTab() {
  ...
  const fetchEnquiries = async () => {
    ...
    try {
      ...
      // Filter by type=vehicle (Automobiles / Two-Wheeler / Four-Wheeler enquiries)
      const res = await fetch(`${API_URL}/admin/enquiries?type=vehicle`, {
      ...
      console.error('Fetch vehicle enquiries error:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm min-h-[60vh] animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Vehicle Enquiries</h2>
          <p className="text-gray-500 mt-1">Enquiries for cars, bikes, scooties and test drives.</p>
        </div>
        <div>
      ...
      {loading ? (
        ...
        <p className="text-gray-500">No vehicle enquiries yet.</p>
      ) : (
